...
cursor_object.execute("CREATE TABLE IF NOT  EXISTS coin(id INTEGER PRIMARY KEY, symbol TEXT, amount INTEGER, price REAL)")
con.commit()
...
def app_nav():
    def clear_all():
        cursor_object.execute("DELETE FROM coin")
        con.commit()
...
def my_portfolio():
    ...
    cursor_object.execute("SELECT * FROM coin")
    coins = cursor_object.fetchall()
    ...
    def insert_coin():
        cursor_object.execute("INSERT INTO coin(symbol, price, amount) VALUES(?, ?, ?)", (symbol_txt.get(), price_txt.get(), amount_txt.get()))
        con.commit()
    ...
    def update_coin():
        cursor_object.execute("UPDATE coin SET symbol=?, price=?, amount=? WHERE id=?", (symbol_update.get(), price_update.get(), amount_update.get(), portid_update.get()))
        con.commit()
    ...
    def delete_coin():
        cursor_object.execute("DELETE FROM coin WHERE id=?", (portid_delete.get(),))
        con.commit()
    ...
    for i in range(0,100):
        for coin in coins:
            if api["data"][i]["symbol"] == coin[1]:
                ...
                pf_id = Label(pycrypto, text = coin[0], bg = "#c4ccc8",fg = "black", font = "Lato 12 ", padx = "2", pady = "2", borderwidth = 2, relief = "groove")
                ...
                no_coins = Label(pycrypto, text = coin[2], bg = "#c4ccc8",fg = "black", font = "Lato 12 ", padx = "2", pady = "2", borderwidth = 2, relief = "groove")
                ...
                current_price = Label(pycrypto, text = "${0:.2f}".format(current_value), bg = "#c4ccc8",fg = "black", font = "Lato 12 ", padx = "2", pady = "2", borderwidth = 2, relief = "groove")
    ...
    total_sp = Label(pycrypto, text = "${0:.2f}".format(total_amount_paid), bg = "#c4ccc8",fg = "black", font = "Lato 12 ", padx = "2", pady = "2", borderwidth = 2, relief = "groove")
    total_sp.grid(row = coin_row,column =4, sticky=N+S+E+W)
    ...
    Refresh = Button(pycrypto, text = 'Refresh', bg = "#01086b",fg = "white", command = my_portfolio, font = "Lato 12 ", padx = "2", pady = "2", borderwidth = 2, relief = "groove")
